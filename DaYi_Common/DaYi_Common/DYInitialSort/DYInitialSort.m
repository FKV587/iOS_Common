...
#import "DYInitialSort.h"

@interface DYInitialSortModel : NSObject
...
@implementation DYInitialSortModel

@end

@implementation DYInitialSort
...
+ (NSMutableDictionary *)sortAndGroupForArray:(NSArray *)ary PropertyName:(NSString *)name {
    ...
    if (ary.count <= 0) {
        [DYHelper showSureAlertControllerMessage:@"数据源不能为空" alert:nil];
        return sortDic;
    }
    ...
    if ([objc isKindOfClass:[NSString class]]) {
        ...
        for (NSString *str in ary){
            DYInitialSortModel *sortString = [DYInitialSortModel new];
        ...
        for (NSObject *obj in ary){
            ...
            for (i = 0; i < propertyCount; i++) {
                ...
                if (i == propertyCount -1) {
                    [DYHelper showSureAlertControllerMessage:[NSString stringWithFormat:@"数据源中的Model没有你指定的属性:%@",name] alert:nil];
                    return sortDic;
                }
            }
        }
    ...
    NSString *itemString;
    for (DYInitialSortModel *sort in sortAry){
...
+ (NSMutableArray *)sortAsInitialWithArray:(NSArray *)ary {
    ...
    for (NSInteger index = 0; index < ary.count; index++) {
        DYInitialSortModel *sortString = ary[index];
        sortString.englishString = [self transform:sortString.string];
