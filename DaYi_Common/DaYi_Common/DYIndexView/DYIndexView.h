...
/** 代理方法 */
@protocol DYIndexViewDelegate <NSObject>
...
/** 数据源方法 */
@protocol DYIndexViewDataSource <NSObject>
...
@interface DYIndexView : UIControl <CAAnimationDelegate>

@property (nonatomic, weak, nullable) id<DYIndexViewDelegate> delegate;
@property (nonatomic, weak, nullable) id<DYIndexViewDataSource> dataSource;
...
- (void)tableView:(UITableView *_Nonnull)tableView willDisplayHeaderView:(UIView *)view forSection:(NSInteger)section;
- (void)tableView:(UITableView *_Nonnull)tableView didEndDisplayingHeaderView:(UIView *)view forSection:(NSInteger)section;
