...
#import "DYIndexView.h"
#import "DYSectionIndicatorView.h"

#import <AudioToolbox/AudioToolbox.h>
#import <objc/runtime.h>

API_AVAILABLE(ios(10.0))
@interface DYIndexView ()

@property (nonatomic, strong) DYSectionIndicatorView *indicatorView;
...
- (void)initialiseAllTitles {
    ...
    for (int i=0; i<self.indexItems.count; i++) {
        ...
        if (title == UITableViewIndexSearch) {
            ...
            //定义图片内容及位置和大小
            attch.image = [UIImage imageWithName:@"icon_user_search"];
        ...
        if (self.isCallback && self.delegate && [self.delegate respondsToSelector:@selector(selectedSectionIndexTitle:atIndex:)]) {
            ...
            if (isLetter) {
                ...
                if (!self.indicatorView) {
                    self.indicatorView = [[DYSectionIndicatorView alloc] initWithFrame:CGRectMake(0, 0, 50, 50)];
                }
